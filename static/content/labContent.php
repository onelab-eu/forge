...
$content_fr = array(
	...
	'congestionControlTitle' => "Contrôle de congestion",
	'serverStudyTitle' => "Etude de la latence d'un serveur web",
	'analysisTitle' => "Analyse des mécanismes TCP",

	'congestionControlContent' => "<p>TCP est utilisé pour le transport fiable de données dans l'Internet. Nous avons précédemment étudié la gestion des connexions et les mécanismes de TCP. Dans les exercices suivants, nous allons nous intéresser à un autre comportement fondamental de TCP : le contrôle de congestion.</p>
					...
					<h2>Débit moyen d'une connexion TCP</h2>
						<p>Supposons que nous souhaitions effectuer un transfert de données de taille importante à travers une connexion TCP.</p>
						<ol>
							<li>
								<p>En négligeant la période pendant laquelle <em>cwnd</em> est plus petite
  que <em>ssthresh</em>, montrez que le débit moyen </em>d</em> associé à une connexion TCP est égal à :</p>
	...
	'serverStudyContent' => "<p>Nous souhaitons étudier la latence liée à la réponse à une requête HTTP. Nous faisons les hypothèses simplificatrices suivantes:
					...
					<ol>
						...
						<li>
							<p>Nous supposons maintenant une fenêtre de congestion <strong>statique</strong> de taille fixe égale à <em>W</em>. Calculez la latence dans ce premier cas :</p>
  							<p class='text-center'>
								<img class='img-responsive img-thumbnail' src='static/img/2.2.jpeg'>
							</p>
						</li>
						<li>
							<p>Nous supposons toujours une fenêtre de congestion <strong>statique</strong> de taille fixe égale à <em>W</em>. Calculez la latence dans ce second cas :</p>
  							<p class='text-center'>
						...
						<li>
							...
							<p><em>K'</em> est le nombre de fenêtre envoyées avant de passer au second cas(<em>log2(1 + RTT * R/S)). Considérez trois cas :</p>
							<ul>
								<li> <em>S</em>=512 octets, <em>RTT</em>= 100msec, O=<strong>100 Koctets</strong> (=200<em>S</em>);</li>
							<table id='tableA' data-toggle='table' class='table table-bordered'>
								<thead>
									<tr>
										<th style>
								...
								<tbody>
									<tr>
										<td style>56 Kbps</td>
										<td><input id='A1-1' type='text' class='form-control'></td>
										<td><input id='A1-2' type='text' class='form-control'></td>
										<td><input id='A1-3' type='text' class='form-control'></td>
										<td><input id='A1-4' type='text' class='form-control'></td>
									</tr>
									<tr>
										<td style>512 Kbps</td>
										<td><input id='A2-1' type='text' class='form-control'></td>
										<td><input id='A2-2' type='text' class='form-control'></td>
										<td><input id='A2-3' type='text' class='form-control'></td>
										<td><input id='A2-4' type='text' class='form-control'></td>
									</tr>
									<tr>
										<td style>8 Mbps</td>
										<td><input id='A3-1' type='text' class='form-control'></td>
										<td><input id='A3-2' type='text' class='form-control'></td>
										<td><input id='A3-3' type='text' class='form-control'></td>
										<td><input id='A3-4' type='text' class='form-control'></td>
									</tr>
									<tr>
										<td style>100 Mbps</td>
										<td><input id='A4-1' type='text' class='form-control'></td>
										<td><input id='A4-2' type='text' class='form-control'></td>
										<td><input id='A4-3' type='text' class='form-control'></td>
										<td><input id='A4-4' type='text' class='form-control'></td>
									</tr>
								</tbody>
							</table>
							<input id='checkTableA' type='submit'/>
								<li> <em>S</em>=512 octets, <em>RTT</em>= 100msec, O=<strong>5 Koctets</strong> (=200<em>S</em>);</li>
							<!--
							<table id='tableB' data-toggle='table' class='table table-bordered'>
								<thead>
									<tr>
										<th style>
											<div class='th-inner'><em>R</em></div>
							...
							-->
								<li> <em>S</em>=512 octets, <em>RTT</em>= <strong>1 seconde</strong>, O=5 Koctets (=200<em>S</em>).</li>
							<!--
							<table id='tableC' data-toggle='table' class='table table-bordered'>
								<thead>
									<tr>
										<th style>
											<div class='th-inner'><em>R</em></div>
	...
	'analysisContent' => "<p>Dans cette partie vous génèrerez des captures de trafic vers un serveur plus ou moins éloigné grâce à la plateforme d'expérimentation <a href='https://www.planet-lab.eu' target='_blank'>PlanetLab Europe</a>. Pour chacune d'elles, tracez précisément le chronogamme et étudiez les mécanismes de contrôle de congestion mis en oeuvre. Discutez en particulier les points suivants:
				<ol>
					<li>
					...
					<li>
						<p>Quel est le débit moyen alors atteint ?</p>
					</li>
					<li>
						<p>Un envoi continu apparaît-il ?</p>
					</li>
					<li>
						<p>Des perturbations sont-elles présentes (déséquencement, retransmissions...) ?</p>
					</li>
				</ol>
			<h2>Génération et capture de trafic HTTP</h2>
			",
	'restart' => 'Recommencer l\'expérience',
	'pleGuideLines' => '<p>Vous trouverez les informations pour réaliser vos traces sur la page <a href="env.html" target="_blank">suivante</a></p>',
);
...
$content_en = array(
	...
	<ol>
		...
		<li>
			...
			<p><em>K'</em> is the number of windows sent before starting the second case(<em>log2(1 + RTT * R/S)). Consider three cases:</p>
				<ul>
					<li> <em>S</em>=512 octets, <em>RTT</em>= 100msec, O=<strong>100 Koctets</strong> (=200<em>S</em>);</li>
						<table id='tableA' data-toggle='table' class='table table-bordered'>
							<thead>
								<tr>
									...
									<th style>
										<div class='th-inner'><em>L</em> (TCP global latency)</div>
									</th>
								</tr>
							</thead>
							<tbody>
					...
					<!--
						<table id='tableB' data-toggle='table' class='table table-bordered'>
							...
							<tbody>
								<tr>
									<td style>56 Kbps</td>
									<td><input id='B1-1' type='text' class='form-control'></td>
									<td><input id='B1-2' type='text' class='form-control'></td>
									<td><input id='B1-3' type='text' class='form-control'></td>
									<td><input id='B1-4' type='text' class='form-control'></td>
								</tr>
								<tr>
									<td style>512 Kbps</td>
									<td><input id='B2-1' type='text' class='form-control'></td>
									<td><input id='B2-2' type='text' class='form-control'></td>
									<td><input id='B2-3' type='text' class='form-control'></td>
									<td><input id='B2-4' type='text' class='form-control'></td>
								</tr>
								<tr>
									<td style>8 Mbps</td>
									<td><input id='B3-1' type='text' class='form-control'></td>
									<td><input id='B3-2' type='text' class='form-control'></td>
									<td><input id='B3-3' type='text' class='form-control'></td>
									<td><input id='B3-4' type='text' class='form-control'></td>
								</tr>
								<tr>
									<td style>100 Mbps</td>
									<td><input id='B4-1' type='text' class='form-control'></td>
									<td><input id='B4-2' type='text' class='form-control'></td>
									<td><input id='B4-3' type='text' class='form-control'></td>
									<td><input id='B4-4' type='text' class='form-control'></td>
								</tr>
							</tbody>
						</table>
					-->
					<li> <em>S</em>=512 octets, <em>RTT</em>= <strong>1 second</strong>, O=5 Koctets (=200<em>S</em>);</li>
					<!--
						<table id='tableC' data-toggle='table' class='table table-bordered'>
							<thead>
								<tr>
	...
	",
	'analysisContent' => "<p>In this part you will generate traffic to a distant server using the <a href='https://www.planet-lab.eu' target='_blank'>PlanetLab Europe</a> testbed. For each one, draw the chronogram and study the congestion control mechanisms put-in-work. Discuss particular of the following</p>
	                <ol>
						<li>
						...
						<li>
							<p>What is the average throughput achieved then ?</p>
						</li>
						<li>
							<p>A continuous sending it appears ?</p>
						</li>
						<li>
							<p>Is there any disturbances (desequencement, retransmission...)?</p>
						</li>
					</ol>
				<h2>Generation and capture of HTTP traffic</h2>",
	'pleGuideLines' => '<p>You will find the informations to create your own traces on the <a href="env.html" target="_blank">following page</a></p>',
	'restart' => 'Restart experiment'
);

include('languageSelection.php');
